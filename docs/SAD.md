...
Hệ thống backend được chia theo Feature-Based Architecture, trong đó mỗi module đại diện cho một domain nghiệp vụ độc lập:
  ...
  - **Auth Module**: X- **Auth Module**: Xử lý đăng ký, đăng nhập truyền thống (Username/Password). Quản lý và xác thực trạng thái phiên làm việc của người dùng thông qua cơ chế Stateful Session (Spring Session / HttpSession).
  ...
  - **Scheduler Module**: Daily Interest Accrual, Debt Status Refresh
...
```
<img src="images/SchedulerSequenceDiagram.png" alt="Sơ đồ kiến trúc" width="100%" />
...
User → Controller → DebtRecommendationService 
  ...
  └─> Map to DTO (DebtRecommendationMapper) → Trả về DebtRecommendationResponseDTO dựa trên chiến lược Snowball hoặc Avalanche.
```
...
User → Controller → AnalysisService (calculateCurrentDti)
  ...
  ├─> Tính toán DTI Ratio
  └─> Đánh giá trạng thái (GOOD / WARNING / CRITICAL) → Trả về DtiResponseDTO sau khi tính toán tỷ lệ nợ trên thu nhập và phân loại sức khỏe tài chính.
```
---
